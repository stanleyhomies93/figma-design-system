...
description: Design UI components in Figma using the Figma MCP server, with strict design-system discipline — auto-layout everywhere, frames over shapes, design tokens bound to every color and typography value, 4/8pt spacing grid, and atomic structure (atoms → molecules → organisms → templates). Use this skill whenever the user asks to "design in Figma", "create a Figma component", "build the Figma file", "mirror the repo to Figma", "reverse-engineer the UI into Figma", or any task that produces Figma frames or components — especially when a frontend repo (React, Vue, HTML/CSS, Tailwind, etc.) is involved as a source of truth. Also use when the user mentions auto-layout, hug/fill, design tokens, Figma variables, atomic design, or wants design-system rules enforced. Use this skill even when the user does not explicitly say "design system" — any Figma component creation task benefits from these rules. Do NOT use for: code generation from existing Figma files (the opposite direction), Figma plugin development, or pure design critique without component creation.
---
